<template>
  <div class="app-container">
    ...
    <div id="tempzhexiantu" style="width: 1000px; height: 600px;float:left;"></div>

    <div style="width: 500; height: 600px;float:right;">
      <el-form :inline="true" class="demo-form-inline">
        <el-tag style="font-size: 22px;">{{name+"的温度检测信息"}}</el-tag>
            <el-table
              :data="list"
              border
              fit
              ...
              style="text-align">
            <el-table-column prop="gmtTempend" label="日期" width="180" />
        ...
        <el-form-item>
          <el-button  @click="black"  type="primary" round>返回</el-button>
        </el-form-item>
      </el-form>
...
<script>
import "@/views/edu/temp/css/tempinfo.css"
...
export default {
  ...
  methods: {
    ...
    getInfo(page = 1) {
      ...
      this.page = page;
      tempApi
        .getUserAllTemp(this.id, this.page, this.limit)
        .then((response) => {
          this.list = response.data.rows; //每页记录数
          this.total = response.data.total; //共多少条
          ...
          for (var i = 0; i < Object.keys(this.list).length; i++) {
            if(this.list[i].temp != null && this.list[i].temp != ''){
                this.YName[count] = this.list[i].temp;
                this.XName[count] = this.list[i].gmtModified.slice(5, 10);
                count++;
            }
          }
    ...
    //折线图初始化
    zhexiantuinit() {
      this.zhexianchart = echarts.init(
        document.getElementById("tempzhexiantu")
      );
      //数据
      //var XName = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"]; //横坐标
      //var data1 = [67, 97, 51, 338, 32, 11, 1]; //纵坐标
      var img = [
      ...
      this.zhexianchart.setOption(this.tempoption);
    },
    mapInit() {
      // 初始化echarts实例
